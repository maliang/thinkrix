...
class ModuleController extends Controller
{
    ...
    {
        return success($this->moduleService->getModules());
    }
    ...
    {
        $moduleDir = app()->getRootPath() . 'app' . DIRECTORY_SEPARATOR . $name;
        $legacyDir = app()->getRootPath() . 'Modules' . DIRECTORY_SEPARATOR . $name;

        $modulePath = null;
        if (is_dir($moduleDir)) { $modulePath = $moduleDir; }
        elseif (is_dir($legacyDir)) { $modulePath = $legacyDir; }
    ...
    protected function installedUi(): array
    {
        ...
        $schema = Card::make()->props(['title' => '已安装模块'])
            ->data(['modules' => [], 'loading' => false, 'routePrefix' => $routePrefix, 'marketVisible' => false])
            ->methods([
                'loadData' => [SetAction::make('loading', true), FetchAction::make('/modules')->get()->then([SetAction::make('modules', '{{ $response.data || [] }}')])->catch([CallAction::make('$message.error', ['{{ $error.message || "加载失败" }}'])])->finally([SetAction::make('loading', false)])],
            ...
            ->children([
                Space::make()->props(['style' => 'margin-bottom: 16px', 'justify' => 'space-between'])->children([
                    Html::div(),
                ...
                DataTable::make()->dataSource('modules')->loading('loading')->rowKey('name')
                    ->columns([
                ...
                Modal::make()->props(['show' => '{{ marketVisible }}', 'title' => '模块商城', 'style' => 'width: 800px', 'preset' => 'card'])
                    ...
                    ->children([
                        Spin::make()->props(['show' => '{{ loading }}'])->children([
                            Result::make()->props(['status' => 'info', 'title' => '敬请期待', 'description' => '模块市场正在开发中，即将上线...'])
                                ->slot('icon', [SvgIcon::make('carbon:store')->props(['class' => 'text-6xl text-primary'])]),
                        ]),
                    ]),
